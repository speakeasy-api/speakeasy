...
func getChangelogs(ctx context.Context, flags GenerateSDKChangelogFlags) error {
	...
	if lang != "" {
		...
		if flags.TargetVersion == "" {
			...
			targetVersions = parseVersionPairs(flags.TargetVersion)
		}

		var previousVersions map[string]string

		if flags.PreviousVersion != "" {
			previousVersions = parseVersionPairs(flags.PreviousVersion)
		}
...
func parseVersionPairs(input string) map[string]string {
	if input == "" {
