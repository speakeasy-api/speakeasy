...
func init() {
	...
	f.SelectSelector.Foreground(styles.Focused.GetForeground()).Bold(true)
	f.MultiSelectSelector.Foreground(styles.Focused.GetForeground()).Bold(true)
	f.SelectedOption.Foreground(styles.Focused.GetForeground()).Bold(true)
	f.SelectedPrefix = lipgloss.NewStyle().Foreground(styles.Focused.GetForeground()).SetString("✓ ").Bold(true)
	...
	f.TextInput.Cursor.Foreground(styles.Colors.WhiteBlackAdaptive)
	f.TextInput.Placeholder.Foreground(styles.Dimmed.GetForeground()).Italic(true)
	f.TextInput.Prompt.Foreground(styles.Colors.WhiteBlackAdaptive)
	f.TextInput.Text.Foreground(styles.Colors.WhiteBlackAdaptive)

	b := &t.Blurred
	b.Description = f.Description.Copy()
	b.TextInput.Placeholder.Italic(true)
	b.Title.Foreground(styles.FocusedDimmed.GetForeground())
	b.TextInput.Text.Foreground(styles.FocusedDimmed.GetForeground())
